...
const Timer = () => {
    ...
    const [worktime,setWorkTime] = useState(25*60) // this is for updating the work timer 
    const [breaktime,setBreakTime] = useState(5*60) // this is for updating the break timer
    ...
    console.log('currentmode: '+currentmode)// this var is for tracking the current moe
          ...
          timer = setInterval(() => {
            ...
            setTime((prevTime) => {
              if (prevTime <= 0) {
                ...
                return currentmode ==='work' ? worktime : breaktime;
              }
    ...
    return (
        <div className='w-[80%] mb-16 mx-auto md:w-full h-full flex justify-center items-center'>
            <div className='md:w-[60%] w-full flex flex-col gap-y-10'>
                ...
                <div className='flex w-full justify-center items-center flex-col mb-6'><div className="text-8xl flex w-full tabular-nums justify-center font-bold text-text-primary text-center md:text-[9rem]">{formatTime(getTime())}</div></div>
                
                <div className='flex justify-center transition-all duration-700 flex-row gap-4'>
                    <button className='flex px-4 flex-grow items-center justify-center py-2 bg-slate-100 rounded-md text-text-accent' onClick={startPomodoro}>
                        {(currentmode == mode) && isRunning ? 
                            ...
                            :
                            (<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" aria-hidden="true" data-slot="icon" className="w-8 h-8 text-text-accent" ><path strokeLinecap="round" strokeLinejoin="round" d="M5.25 5.653c0-.856.917-1.398 1.667-.986l11.54 6.347a1.125 1.125 0 0 1 0 1.972l-11.54 6.347a1.125 1.125 0 0 1-1.667-.986V5.653Z"></path></svg>)}
                    </button>
